...
<form action="{{ url('/admin/holiday/update') }}" method="POST" enctype="multipart/form-data">
  ...
  <input type="hidden" class="form-control" name="holiday_id" value="{{ $holiday->id }}" required>
...
<label for="formGroupExampleInput3">Holiday Title <span class="text-danger">*</span></label>
	<input type="text" name="holiday_title" value="{{ $holiday->holiday_title }}" class="form-control" placeholder="Holiday Title" required></input>
</div>
  ...
  <div class="form-group">
    ...
    <select class="form-control" name="holiday_type" required> 
      <option slected>{{ $holiday->holiday_type }}</option>
...
<label for="formGroupExampleInput3">Holiday File</label>
  ...
  <a href="{{ $holiday->holiday_file }}"> {{ $holiday->holiday_file }}</a>
</div>
...
<label for="formGroupExampleInput3">Holiday Start</label>
	<input type="date" name="start_date" value="{{ substr($holiday->start_date, 0, 10) }}" class="form-control"></input>
</div>
</div>
...
<label for="formGroupExampleInput3">Holiday End</label>
	<input type="date" name="end_date" value="{{ substr($holiday->end_date, 0, 10) }}"  class="form-control"></input>
</div>
...
<label for="formGroupExampleInput3">Holiday Details</label>
	<textarea name="holiday_message" id="div_editor1">{!! $holiday->holiday_message !!}</textarea>
